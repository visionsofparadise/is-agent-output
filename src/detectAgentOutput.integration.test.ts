...
itBash("returns exit 0 when bash-spawned cli stdout is captured by the test runner", async () => {
	ensureBuild();
...
itBash("returns exit 1 when the same invocation is piped to cat", async () => {
	ensureBuild();
...
itBash("returns exit 1 when the same invocation is redirected to a file", async () => {
	ensureBuild();
...
itBash("returns exit 1 when an intermediary node process captures the cli", async () => {
	ensureBuild();
	mkdirSync(dirname(fixtureDistPath), { recursive: true });
	copyFileSync(fixtureSourcePath, fixtureDistPath);
	const result = await exitCodeOf(`${quoted(posixPathOf(process.execPath))} ${quoted(posixPathOf(fixtureDistPath))}`);
