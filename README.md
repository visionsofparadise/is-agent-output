...
- A redirect an agent writes inside a script file, or applies with `exec >`, never reaches a relay's command line and is invisible to the file branch. A relay command line naming a script file it does not source therefore fails the sink closed with `unreadable script in a relay command line`. A sourced script is exempt, since it runs in the relay whose command line was read, so a harness that sources setup — Claude Code's shell snapshot — still resolves.
